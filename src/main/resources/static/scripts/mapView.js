var map = L.map('map').setView([37.8715, -122.2608], 15); // Berkeley default view
...
function searchLocation() {
    ...
    if (searchTerm.trim() !== '') {
        ...
        fetch(`/api/incidents/?title=${encodeURIComponent(searchTerm)}`)
...
document.getElementById("incidentForm").addEventListener("submit", function (event) {
    ...
    if (!clickedLocation) {
        alert('Click the map to choose an incident location first.');
        return;
    }
